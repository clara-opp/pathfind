...
def fetch_exchange_rates() -> pd.DataFrame:
    ...
    rates = pd.DataFrame(data["exchange_rates"])
    return rates


def fetch_country_indices_all() -> pd.DataFrame:
    """
    Fetch country-level indices for all countries in numbeo_countries.csv.
    Uses iso3 as query parameter where possible.
    """
    countries_df = pd.read_csv("numbeo_countries.csv")
...
if __name__ == "__main__":
    ...
    try:
        ...
        rates_df.to_csv("numbeo_exchange_rates.csv", index=False)
        print("Saved exchange rates to 'numbeo_exchange_rates.csv'.")
    except Exception as e:
        print(f"Error fetching exchange rates: {e}")

    # Country indices (loop over all countries)
    try:
        indices_df = fetch_country_indices_all()
        indices_df.to_csv("numbeo_country_indices.csv", index=False)
        print("Saved country indices to 'numbeo_country_indices.csv'.")
